...
# 公開ディレクトリ
pubdir = "/home/j1406/public_html/data_csv_1day"
...
while (time_list[idx0] + 1 < time_list[-1]) do 
  ...
  unless ( idx2 )
    num.times do |i|
      ...
      meanday[i] = vars_list_narray[i][idx0+9..idx0+16].mean(0)
  ...
  csv = open("#{pubdir}/#{myid}_meanday.csv", "a")
  csv.puts "#{time0.strftime("%Y/%m/%d")},#{meanday.join(',')},\n"
  csv.close
