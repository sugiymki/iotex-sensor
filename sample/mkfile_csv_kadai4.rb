...
# 公開ディレクトリ
pubdir = "/iotex/graph_1month/#{myid}/csv"
...
while (time_list[idx0] + 1 < time_list[-1]) do 
  ...
  # ファイルの書き出し (平均値)
  csv = open("#{pubdir}/#{myid}_mean_kadai4.csv", "a")
  csv.puts "#{time0.strftime("%Y/%m/%d")},#{mean.join(',')},\n"
  csv.close
  csv = open("#{pubdir}/#{myid}_mean2_kadai4.csv", "a")
  csv.puts "#{time0.strftime("%Y/%m/%d")},#{mean2.join(',')},\n"
  csv.close
  csv = open("#{pubdir}/#{myid}_min_kadai4.csv", "a")
  csv.puts "#{time0.strftime("%Y/%m/%d")},#{min.join(',')},\n"
  csv.close
  csv = open("#{pubdir}/#{myid}_max_kadai4.csv", "a")
